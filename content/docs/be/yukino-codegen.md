...
### 请描述 YukinoCodegen 的整体架构分层和核心模块职责

A:

项目是单二进制 Go 程序 (`cmd/yukino-codegen`) , 核心按职责分为七块:
...
7. 交互层: bubbletea TUI (`internal/tui`) 、`-p` 非交互模式、基于自研 `yukino_http` 的 `--remote` WebSocket 服务 (`internal/remote`) .
...
`cmd/yukino-codegen/main.go:37` 按 flag 分发四种模式:

1. Teammate 工作进程 (`--teammate`) : 被团队 Lead 通过 tmux/iTerm 拉起的无头工作进程, 任务从文件邮箱读取, 事件流打到 stderr 供终端面板展示 (`cmd/yukino-codegen/teammate.go`) .
...
Anthropic Prompt Cache 要求前缀字节完全一致才能命中. YukinoCodegen 的策略:
...
Yukino 方式: LLM 输出过程中, 每个 `ToolCallComplete` 事件立即提交给 Executor.
...
hooks:
  - event: pre_tool_use
    ...
    action:
      type: command
      command: "gofmt -w ${YukinoCodegen_FILE_PATH}"
    on_error: ignore
```
...
1. `command`: 执行 shell 命令 (注入 `YukinoCodegen_EVENT`, `YukinoCodegen_TOOL`, `YukinoCodegen_FILE_PATH` 环境变量)
...
1. 启动时 `InjectLongTermMemory` 把指令 (YukinoCodegen.md) + 记忆内容以 system-reminder 形式一次性前插到对话头部 (conversation.go:147)
...
记忆整合 (consolidation) 可能被多个 YukinoCodegen 进程同时触发, 用 PID 文件锁互斥 (`internal/memory/consolidation/lock.go:62`) :
...
后两者用 `yukino-codegen --teammate` 拉起独立进程各占一个终端面板, 进程崩溃互不影响且用户可直接观察每个队友.
...
Prompt Cache 和上下文预算存在天然张力: 预算裁剪要改写历史消息, 而缓存要求前缀字节不变. Yukino 的协同策略:
...
### 如果要为 Yukino 添加 "Undo/Redo" 功能 (撤销 Agent 的文件修改) , 你会如何设计?
...
### 如何设计一个分布式 Agent 系统 (多机协作) ? Yukino 的 Teams 系统如何扩展到分布式场景?
...
### Yukino 的 Prompt 构建系统如何保证可维护性和可扩展性?
...
sections := []Section{
    {"Identity", 0, "You are YukinoCodegen..."},
...
注意 custom_instructions (YukinoCodegen.md) 、skills 清单、memory 不是 system prompt section: 它们是项目级的, 放进 system prompt 会让缓存前缀随项目变化, 因此由 `conversation.InjectLongTermMemory` 作为会话首条 system-reminder 消息注入, 且整个会话只注入一次 (conversation.go:147) .
