...
第二, 业务自定义指标是核心价值. 通用 SDK 采集的是 FCP、LCP、TTFB 这类标准指标, 但企业项目真正要回答的问题是"lark 框架启动花了多久""四个菜单接口里哪个最慢""某个 bizCode 下的页面卡不卡". 这些指标与业务架构深度绑定, 任何通用 SDK 都不提供, 最终还是要手写采集逻辑. 与其在 SDK 之上再包一层定制, 不如直接手写.
...
第五, 存量业务友好度. 这一点与第 3 节手写 SWR 的理由同构: onebp 是 seajs 加 Lark 的老架构, 现代监控 SDK 普遍假设 npm 构建、ESM、SPA 路由, 在 CMD 模块加载器和 hash 路由的环境下要么接不进去, 要么拿不到正确的页面维度( boot.ts 里要手动从 location.hash 解析 pathname 和 bizCode) . 手写代码没有环境假设, 能在任何能执行 JS 的地方工作.
