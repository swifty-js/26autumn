...
- [1. Go基础面试题](#_1-go基础面试题)
  - [1.1 与其他语言相比，使用 Go 有什么好处?](#_1-1-与其他语言相比-使用-go-有什么好处)
  - [1.2 什么是协程？](#_1-2-什么是协程)
  - [1.3 协程和线程和进程的区别？](#_1-3-协程和线程和进程的区别)
  - [1.4 Golang 中 make 和 new 的区别？](#_1-4-golang-中-make-和-new-的区别)
  - [1.5 Golang 中数组和切片的区别？](#_1-5-golang-中数组和切片的区别)
  - [1.6 使用for range 的时候，它的地址会发生变化吗？](#_1-6-使用for-range-的时候-它的地址会发生变化吗)
  - [1.7 如何高效地拼接字符串？](#_1-7-如何高效地拼接字符串)
  - [1.8 defer 的执行顺序是怎样的？defer 的作用或者使用场景是什么?](#_1-8-defer-的执行顺序是怎样的-defer-的作用或者使用场景是什么)
  - [1.9 什么是 rune 类型？](#_1-9-什么是-rune-类型)
  - [1.10 Go 语言 tag 有什么用？](#_1-10-go-语言-tag-有什么用)
  - [1.11 go 打印时 %v %+v %#v 的区别？](#_1-11-go-打印时-v-v-v-的区别)
  - [1.12 Go语言中空 struct{} 占用空间么？](#_1-12-go语言中空-struct-占用空间么)
  - [1.13 Go语言中，空 struct{} 有什么用？](#_1-13-go语言中-空-struct-有什么用)
  - [1.14 init() 函数是什么时候执行的？](#_1-14-init-函数是什么时候执行的)
  - [1.15 2 个 interface 可以比较吗 ？](#_1-15-2-个-interface-可以比较吗)
  - [1.16 2 个 nil 可能不相等吗？](#_1-16-2-个-nil-可能不相等吗)
  - [1.17 Go 语言函数传参是值类型还是引用类型？](#_1-17-go-语言函数传参是值类型还是引用类型)
  - [1.18 如何知道一个对象是分配在栈上还是堆上？](#_1-18-如何知道一个对象是分配在栈上还是堆上)
  - [1.19 Go语言的多返回值是如何实现的？](#_1-19-go语言的多返回值是如何实现的)
  - [1.20 Go语言中"\_"的作用](#_1-20-go语言中-_-的作用)
  - [1.21 Go语言普通指针和unsafe.Pointer有什么区别？](#_1-21-go语言普通指针和unsafe-pointer有什么区别)
  - [1.22 unsafe.Pointer与uintptr有什么区别和联系](#_1-22-unsafe-pointer与uintptr有什么区别和联系)
- [2. Slice面试题](#_2-slice面试题)
  - [2.1 slice的底层结构是怎样的？](#_2-1-slice的底层结构是怎样的)
  - [2.2 Go语言里slice是怎么扩容的？](#_2-2-go语言里slice是怎么扩容的)
  - [2.3 从一个切片截取出另一个切片，修改新切片的值会影响原来的切片内容吗](#_2-3-从一个切片截取出另一个切片-修改新切片的值会影响原来的切片内容吗)
  - [2.4 slice作为函数参数传递，会改变原slice吗？](#_2-4-slice作为函数参数传递-会改变原slice吗)
- [3. Map面试题](#_3-map面试题)
  - [3.1 Go语言Map的底层实现原理是怎样的？](#_3-1-go语言map的底层实现原理是怎样的)
  - [3.2 Go语言Map的遍历是有序的还是无序的？](#_3-2-go语言map的遍历是有序的还是无序的)
  - [3.3 Go语言Map的遍历为什么要设计成无序的？](#_3-3-go语言map的遍历为什么要设计成无序的)
  - [3.4 Map如何实现顺序读取？](#_3-4-map如何实现顺序读取)
  - [3.5 Go语言的Map是否是并发安全的？](#_3-5-go语言的map是否是并发安全的)
  - [3.6 Map的Key一定要是可比较的吗？为什么？](#_3-6-map的key一定要是可比较的吗-为什么)
  - [3.7 Go语言Map的扩容时机是怎样的？](#_3-7-go语言map的扩容时机是怎样的)
  - [3.8 Go语言Map的扩容过程是怎样的？](#_3-8-go语言map的扩容过程是怎样的)
  - [3.9 可以对Map的元素取地址吗？](#_3-9-可以对map的元素取地址吗)
  - [3.10 Map 中删除一个 key，它的内存会释放么？](#_3-10-map-中删除一个-key-它的内存会释放么)
  - [3.11 Map可以边遍历边删除吗](#_3-11-map可以边遍历边删除吗)
- [4. Channel面试题](#_4-channel面试题)
  - [4.1 什么是CSP？](#_4-1-什么是csp)
  - [4.2 Channel的底层实现原理是怎样的？](#_4-2-channel的底层实现原理是怎样的)
  - [4.3 向channel发送数据的过程是怎样的？](#_4-3-向channel发送数据的过程是怎样的)
  - [4.4 从Channel读取数据的过程是怎样的？](#_4-4-从channel读取数据的过程是怎样的)
  - [4.5 从一个已关闭Channel仍能读出数据吗？](#_4-5-从一个已关闭channel仍能读出数据吗)
  - [4.6 Channel在什么情况下会引起内存泄漏？](#_4-6-channel在什么情况下会引起内存泄漏)
  - [4.7 关闭Channel会产生异常吗？](#_4-7-关闭channel会产生异常吗)
  - [4.8 往一个关闭的Channel写入数据会发生什么？](#_4-8-往一个关闭的channel写入数据会发生什么)
  - [4.9 什么是select？](#_4-9-什么是select)
  - [4.10 select的执行机制是怎样的？](#_4-10-select的执行机制是怎样的)
  - [4.11 select的实现原理是怎样的？](#_4-11-select的实现原理是怎样的)
- [5. Sync面试题](#_5-sync面试题)
  - [5.1 除了 mutex 以外还有那些方式安全读写共享变量？](#_5-1-除了-mutex-以外还有那些方式安全读写共享变量)
  - [5.2 Go 语言是如何实现原子操作的？](#_5-2-go-语言是如何实现原子操作的)
  - [5.3 聊聊原子操作和锁的区别？](#_5-3-聊聊原子操作和锁的区别)
  - [5.4 Go语言互斥锁mutex底层是怎么实现的？](#_5-4-go语言互斥锁mutex底层是怎么实现的)
  - [5.5 Mutex 有几种模式？](#_5-5-mutex-有几种模式)
  - [5.6 在Mutex上自旋的goroutine 会占用太多资源吗](#_5-6-在mutex上自旋的goroutine-会占用太多资源吗)
  - [5.7 Mutex 已经被一个 Goroutine 获取了, 其它等待中的 Goroutine 们只能一直等待。那么等这个锁释放后，等待中的 Goroutine 中哪一个会优先获取 Mutex 呢?](#_5-7-mutex-已经被一个-goroutine-获取了-其它等待中的-goroutine-们只能一直等待-那么等这个锁释放后-等待中的-goroutine-中哪一个会优先获取-mutex-呢)
  - [5.8 sync.Once 的作用是什么，讲讲它的底层实现原理？](#_5-8-sync-once-的作用是什么-讲讲它的底层实现原理)
  - [5.9 WaitGroup 是怎样实现协程等待？](#_5-9-waitgroup-是怎样实现协程等待)
  - [5.10 讲讲sync.Map的底层原理](#_5-10-讲讲sync-map的底层原理)
  - [5.11 read map和dirty map之间有什么关联？](#_5-11-read-map和dirty-map之间有什么关联)
  - [5.12 为什么要设计nil和expunged两种删除状态？](#_5-12-为什么要设计nil和expunged两种删除状态)
  - [5.13 sync.Map 适用的场景？](#_5-13-sync-map-适用的场景)
- [6. Context面试题](#_6-context面试题)
  - [6.1 Go语言里的Context是什么？](#_6-1-go语言里的context是什么)
  - [6.2 Go语言的Context有什么作用？](#_6-2-go语言的context有什么作用)
  - [6.3 Context.Value的查找过程是怎样的](#_6-3-context-value的查找过程是怎样的)
  - [6.4 Context如何被取消](#_6-4-context如何被取消)
- [7. Interface面试题](#_7-interface面试题)
  - [7.1 Go语言中，interface的底层原理是怎样的？](#_7-1-go语言中-interface的底层原理是怎样的)
  - [7.2 iface和eface的区别是什么？](#_7-2-iface和eface的区别是什么)
  - [7.3 类型转换和断言的区别是什么？](#_7-3-类型转换和断言的区别是什么)
  - [7.4 Go语言interface有哪些应用场景](#_7-4-go语言interface有哪些应用场景)
  - [7.5 接口之间可以相互比较吗？](#_7-5-接口之间可以相互比较吗)
- [8. 反射面试题](#_8-反射面试题)
  - [8.1 什么是反射？](#_8-1-什么是反射)
  - [8.2 Go语言如何实现反射？](#_8-2-go语言如何实现反射)
  - [8.3 Go语言中的反射应用有哪些](#_8-3-go语言中的反射应用有哪些)
  - [8.4 如何比较两个对象完全相同](#_8-4-如何比较两个对象完全相同)
- [9. GMP面试题](#_9-gmp面试题)
  - [9.1 Go语言的GMP模型是什么？](#_9-1-go语言的gmp模型是什么)
  - [9.2 什么是Go scheduler](#_9-2-什么是go-scheduler)
  - [9.3 Go语言在进行goroutine调度的时候，调度策略是怎样的？](#_9-3-go语言在进行goroutine调度的时候-调度策略是怎样的)
  - [9.4 发生调度的时机有哪些？](#_9-4-发生调度的时机有哪些)
  - [9.5 M寻找可运行G的过程是怎样的？](#_9-5-m寻找可运行g的过程是怎样的)
  - [9.6 GMP能不能去掉P层？会怎么样？](#_9-6-gmp能不能去掉p层-会怎么样)
  - [9.7 P和M在什么时候会被创建？](#_9-7-p和m在什么时候会被创建)
  - [9.8 m0是什么，有什么用](#_9-8-m0是什么-有什么用)
  - [9.9 g0是一个怎样的协程，有什么用？](#_9-9-g0是一个怎样的协程-有什么用)
  - [9.10 g0栈和用户栈是如何进行切换的？](#_9-10-g0栈和用户栈是如何进行切换的)
- [10. 内存管理面试题](#_10-内存管理面试题)
  - [10.1 讲讲Go语言是如何分配内存的？](#_10-1-讲讲go语言是如何分配内存的)
  - [10.2 知道 golang 的内存逃逸吗？什么情况下会发生内存逃逸？](#_10-2-知道-golang-的内存逃逸吗-什么情况下会发生内存逃逸)
  - [10.3 内存逃逸有什么影响？](#_10-3-内存逃逸有什么影响)
  - [10.4 Channel是分配在栈上，还是堆上？](#_10-4-channel是分配在栈上-还是堆上)
  - [10.5 Go语言在什么情况下会发生内存泄漏？](#_10-5-go语言在什么情况下会发生内存泄漏)
  - [10.6 Go语言发生了内存泄漏如何定位和优化？](#_10-6-go语言发生了内存泄漏如何定位和优化)
- [11. 垃圾回收面试题](#_11-垃圾回收面试题)
  - [11.1 常见的 GC 实现方式有哪些？](#_11-1-常见的-gc-实现方式有哪些)
  - [11.2 Go 语言的 GC 使用的是什么？](#_11-2-go-语言的-gc-使用的是什么)
  - [11.3 三色标记法是什么？](#_11-3-三色标记法是什么)
  - [11.4 Go语言GC的根对象到底是什么？](#_11-4-go语言gc的根对象到底是什么)
  - [11.5 STW 是什么意思？](#_11-5-stw-是什么意思)
  - [11.6 并发标记清除法的难点是什么？](#_11-6-并发标记清除法的难点是什么)
  - [11.7 Go语言是如何解决并发标记清除时，用户程序并发修改对象引用问题的？](#_11-7-go语言是如何解决并发标记清除时-用户程序并发修改对象引用问题的)
  - [11.8 什么是写屏障、混合写屏障，如何实现？](#_11-8-什么是写屏障-混合写屏障-如何实现)
  - [11.9 Go 语言中 GC 的流程是什么？](#_11-9-go-语言中-gc-的流程是什么)
  - [11.10 GC触发的时机有哪些？](#_11-10-gc触发的时机有哪些)
  - [11.11 GC 关注的指标有哪些？](#_11-11-gc-关注的指标有哪些)
  - [11.12 有了 GC，为什么还会发生内存泄露？](#_11-12-有了-gc-为什么还会发生内存泄露)
  - [11.13 Go 的 GC 如何调优？](#_11-13-go-的-gc-如何调优)
  - [11.14 如何观察 Go GC？](#_11-14-如何观察-go-gc)
- [12. Go代码面试题](#_12-go代码面试题)
  - [12.1 开启100个协程，顺序打印1-1000，且保证协程号1的，打印尾数为1的数字](#_12-1-开启100个协程-顺序打印1-1000-且保证协程号1的-打印尾数为1的数字)
  - [12.2 三个goroutinue交替打印abc 10次](#_12-2-三个goroutinue交替打印abc-10次)
  - [12.3 用不超过10个goroutine不重复的打印slice中的100个元素](#_12-3-用不超过10个goroutine不重复的打印slice中的100个元素)
  - [12.4 两个协程交替打印奇偶数](#_12-4-两个协程交替打印奇偶数)
  - [12.5 用单个channel实现0,1的交替打印](#_12-5-用单个channel实现0-1的交替打印)
  - [12.6 sync.Cond实现多生产者多消费者](#_12-6-sync-cond实现多生产者多消费者)
  - [12.7 使用go实现1000个并发控制并设置执行超时时间1秒](#_12-7-使用go实现1000个并发控制并设置执行超时时间1秒)
  - [12.8 使用两个Goroutine，向标准输出中按顺序按顺序交替打出字母与数字，输出是a1b2c3](#_12-8-使用两个goroutine-向标准输出中按顺序按顺序交替打出字母与数字-输出是a1b2c3)
  - [12.9 编写一个程序限制10个goroutine执行，每执行完一个goroutine就放一个新的goroutine进来](#_12-9-编写一个程序限制10个goroutine执行-每执行完一个goroutine就放一个新的goroutine进来)
...
而unsafe.Pointer是Go的通用指针类型，可以理解为C语言中的`void*`，它绕过了Go的类型系统。unsafe.Pointer可以与任意类型的指针相互转换，也可以与uintptr进行转换来做指针运算。
...
map的就是一个hmap的结构。Go Map的底层实现是一个哈希表。它在运行时表现为一个指向`hmap`结构体的指针，`hmap`中记录了桶数组指针`buckets`、溢出桶指针以及元素个数等字段。每个桶是一个`bmap`结构体，能存储8个键值对和8个`tophash`，并有指向下一个溢出桶的指针`overflow`。为了内存紧凑，`bmap`中采用的是先存8个键再存8个值的存储方式。
...
`sync.Once`的作用是确保一个函数在程序生命周期内，无论在多少个goroutine中被调用，都只会被执行一次。它常用于单例对象的初始化或一些只需要执行一次的全局配置加载
...
- 低32位: 作为等待者计数器（waiter count），记录了调用`Wait()`方法后被阻塞的 goroutine 的数量。
...
`sync.Map`的底层核心是"空间换时间"，通过两个Map（`read`和`dirty`）\*\* 的冗余结构，实现"读写分离"，最终达到针对特定场景的"读"操作无锁优化。
...
Go scheduler就是Go运行时的协程调度器，负责在系统线程上调度执行goroutine。它 是 Go runtime 的一部分，它内嵌在 Go 程序里，和 Go 程序一起运行。它的主要工作是决定哪个goroutine在哪个线程上运行，以及何时进行上下文切换。scheduler的核心是`schedule()`函数，它在无限循环中寻找可运行的goroutine。当找到后通过`execute()`函数切换到goroutine执行，goroutine主动让出或被抢占时再回到调度循环。
...
P的创建时机：P在调度器初始化时一次性创建。在`schedinit()`函数中会调用`procresize()`，根据`GOMAXPROCS`值创建对应数量的P对象，存储在全局的`allp`数组中。之后P的数量基本固定，只有在调用`runtime.GOMAXPROCS()`动态调整时才会重新分配P。
...
核心作用：g0专门负责执行调度逻辑，包括goroutine的创建、销毁、调度决策等。当M需要进行调度时，会从当前运行的用户goroutine切换到g0上执行`schedule()`函数。g0还负责处理垃圾回收、栈扫描、信号处理等运行时操作。
