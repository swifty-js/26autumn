...
- [1. Go基础面试题](#1-go基础面试题)
  - [1.1 与其他语言相比，使用 Go 有什么好处?](#11-与其他语言相比使用-go-有什么好处)
  - [1.2 什么是协程？](#12-什么是协程)
  - [1.3 协程和线程和进程的区别？](#13-协程和线程和进程的区别)
  - [1.4 Golang 中 make 和 new 的区别？](#14-golang-中-make-和-new-的区别)
  - [1.5 Golang 中数组和切片的区别？](#15-golang-中数组和切片的区别)
  - [1.6 使用for range 的时候，它的地址会发生变化吗？](#16-使用for-range-的时候它的地址会发生变化吗)
  - [1.7 如何高效地拼接字符串？](#17-如何高效地拼接字符串)
  - [1.8 defer 的执行顺序是怎样的？defer 的作用或者使用场景是什么?](#18-defer-的执行顺序是怎样的defer-的作用或者使用场景是什么)
  - [1.9 什么是 rune 类型？](#19-什么是-rune-类型)
  - [1.10 Go 语言 tag 有什么用？](#110-go-语言-tag-有什么用)
  - [1.11 go 打印时 %v %+v %#v 的区别？](#111-go-打印时-v-v-v-的区别)
  - [1.12 Go语言中空 struct{} 占用空间么？](#112-go语言中空-struct-占用空间么)
  - [1.13 Go语言中，空 struct{} 有什么用？](#113-go语言中空-struct-有什么用)
  - [1.14 init() 函数是什么时候执行的？](#114-init-函数是什么时候执行的)
  - [1.15 2 个 interface 可以比较吗 ？](#115-2-个-interface-可以比较吗)
  - [1.16 2 个 nil 可能不相等吗？](#116-2-个-nil-可能不相等吗)
  - [1.17 Go 语言函数传参是值类型还是引用类型？](#117-go-语言函数传参是值类型还是引用类型)
  - [1.18 如何知道一个对象是分配在栈上还是堆上？](#118-如何知道一个对象是分配在栈上还是堆上)
  - [1.19 Go语言的多返回值是如何实现的？](#119-go语言的多返回值是如何实现的)
  - [1.20 Go语言中"\_"的作用](#120-go语言中_的作用)
  - [1.21 Go语言普通指针和unsafe.Pointer有什么区别？](#121-go语言普通指针和unsafepointer有什么区别)
  - [1.22 unsafe.Pointer与uintptr有什么区别和联系](#122-unsafepointer与uintptr有什么区别和联系)
- [2. Slice面试题](#2-slice面试题)
  - [2.1 slice的底层结构是怎样的？](#21-slice的底层结构是怎样的)
  - [2.2 Go语言里slice是怎么扩容的？](#22-go语言里slice是怎么扩容的)
  - [2.3 从一个切片截取出另一个切片，修改新切片的值会影响原来的切片内容吗](#23-从一个切片截取出另一个切片修改新切片的值会影响原来的切片内容吗)
  - [2.4 slice作为函数参数传递，会改变原slice吗？](#24-slice作为函数参数传递会改变原slice吗)
- [3. Map面试题](#3-map面试题)
  - [3.1 Go语言Map的底层实现原理是怎样的？](#31-go语言map的底层实现原理是怎样的)
  - [3.2 Go语言Map的遍历是有序的还是无序的？](#32-go语言map的遍历是有序的还是无序的)
  - [3.3 Go语言Map的遍历为什么要设计成无序的？](#33-go语言map的遍历为什么要设计成无序的)
  - [3.4 Map如何实现顺序读取？](#34-map如何实现顺序读取)
  - [3.5 Go语言的Map是否是并发安全的？](#35-go语言的map是否是并发安全的)
  - [3.6 Map的Key一定要是可比较的吗？为什么？](#36-map的key一定要是可比较的吗为什么)
  - [3.7 Go语言Map的扩容时机是怎样的？](#37-go语言map的扩容时机是怎样的)
  - [3.8 Go语言Map的扩容过程是怎样的？](#38-go语言map的扩容过程是怎样的)
  - [3.9 可以对Map的元素取地址吗？](#39-可以对map的元素取地址吗)
  - [3.10 Map 中删除一个 key，它的内存会释放么？](#310-map-中删除一个-key它的内存会释放么)
  - [3.11 Map可以边遍历边删除吗](#311-map可以边遍历边删除吗)
- [4. Channel面试题](#4-channel面试题)
  - [4.1 什么是CSP？](#41-什么是csp)
  - [4.2 Channel的底层实现原理是怎样的？](#42-channel的底层实现原理是怎样的)
  - [4.3 向channel发送数据的过程是怎样的？](#43-向channel发送数据的过程是怎样的)
  - [4.4 从Channel读取数据的过程是怎样的？](#44-从channel读取数据的过程是怎样的)
  - [4.5 从一个已关闭Channel仍能读出数据吗？](#45-从一个已关闭channel仍能读出数据吗)
  - [4.6 Channel在什么情况下会引起内存泄漏？](#46-channel在什么情况下会引起内存泄漏)
  - [4.7 关闭Channel会产生异常吗？](#47-关闭channel会产生异常吗)
  - [4.8 往一个关闭的Channel写入数据会发生什么？](#48-往一个关闭的channel写入数据会发生什么)
  - [4.9 什么是select？](#49-什么是select)
  - [4.10 select的执行机制是怎样的？](#410-select的执行机制是怎样的)
  - [4.11 select的实现原理是怎样的？](#411-select的实现原理是怎样的)
- [5. Sync面试题](#5-sync面试题)
  - [5.1 除了 mutex 以外还有那些方式安全读写共享变量？](#51-除了-mutex-以外还有那些方式安全读写共享变量)
  - [5.2 Go 语言是如何实现原子操作的？](#52-go-语言是如何实现原子操作的)
  - [5.3 聊聊原子操作和锁的区别？](#53-聊聊原子操作和锁的区别)
  - [5.4 Go语言互斥锁mutex底层是怎么实现的？](#54-go语言互斥锁mutex底层是怎么实现的)
  - [5.5 Mutex 有几种模式？](#55-mutex-有几种模式)
  - [5.6 在Mutex上自旋的goroutine 会占用太多资源吗](#56-在mutex上自旋的goroutine-会占用太多资源吗)
  - [5.7 Mutex 已经被一个 Goroutine 获取了, 其它等待中的 Goroutine 们只能一直等待。那么等这个锁释放后，等待中的 Goroutine 中哪一个会优先获取 Mutex 呢?](#57-mutex-已经被一个-goroutine-获取了-其它等待中的-goroutine-们只能一直等待那么等这个锁释放后等待中的-goroutine-中哪一个会优先获取-mutex-呢)
  - [5.8 sync.Once 的作用是什么，讲讲它的底层实现原理？](#58-synconce-的作用是什么讲讲它的底层实现原理)
  - [5.9 WaitGroup 是怎样实现协程等待？](#59-waitgroup-是怎样实现协程等待)
  - [5.10 讲讲sync.Map的底层原理](#510-讲讲syncmap的底层原理)
  - [5.11 read map和dirty map之间有什么关联？](#511-read-map和dirty-map之间有什么关联)
  - [5.12 为什么要设计nil和expunged两种删除状态？](#512-为什么要设计nil和expunged两种删除状态)
  - [5.13 sync.Map 适用的场景？](#513-syncmap-适用的场景)
- [6. Context面试题](#6-context面试题)
  - [6.1 Go语言里的Context是什么？](#61-go语言里的context是什么)
  - [6.2 Go语言的Context有什么作用？](#62-go语言的context有什么作用)
  - [6.3 Context.Value的查找过程是怎样的](#63-contextvalue的查找过程是怎样的)
  - [6.4 Context如何被取消](#64-context如何被取消)
- [7. Interface面试题](#7-interface面试题)
  - [7.1 Go语言中，interface的底层原理是怎样的？](#71-go语言中interface的底层原理是怎样的)
  - [7.2 iface和eface的区别是什么？](#72-iface和eface的区别是什么)
  - [7.3 类型转换和断言的区别是什么？](#73-类型转换和断言的区别是什么)
  - [7.4 Go语言interface有哪些应用场景](#74-go语言interface有哪些应用场景)
  - [7.5 接口之间可以相互比较吗？](#75-接口之间可以相互比较吗)
- [8. 反射面试题](#8-反射面试题)
  - [8.1 什么是反射？](#81-什么是反射)
  - [8.2 Go语言如何实现反射？](#82-go语言如何实现反射)
  - [8.3 Go语言中的反射应用有哪些](#83-go语言中的反射应用有哪些)
  - [8.4 如何比较两个对象完全相同](#84-如何比较两个对象完全相同)
- [9. GMP面试题](#9-gmp面试题)
  - [9.1 Go语言的GMP模型是什么？](#91-go语言的gmp模型是什么)
  - [9.2 什么是Go scheduler](#92-什么是go-scheduler)
  - [9.3 Go语言在进行goroutine调度的时候，调度策略是怎样的？](#93-go语言在进行goroutine调度的时候调度策略是怎样的)
  - [9.4 发生调度的时机有哪些？](#94-发生调度的时机有哪些)
  - [9.5 M寻找可运行G的过程是怎样的？](#95-m寻找可运行g的过程是怎样的)
  - [9.6 GMP能不能去掉P层？会怎么样？](#96-gmp能不能去掉p层会怎么样)
  - [9.7 P和M在什么时候会被创建？](#97-p和m在什么时候会被创建)
  - [9.8 m0是什么，有什么用](#98-m0是什么有什么用)
  - [9.9 g0是一个怎样的协程，有什么用？](#99-g0是一个怎样的协程有什么用)
  - [9.10 g0栈和用户栈是如何进行切换的？](#910-g0栈和用户栈是如何进行切换的)
- [10. 内存管理面试题](#10-内存管理面试题)
  - [10.1 讲讲Go语言是如何分配内存的？](#101-讲讲go语言是如何分配内存的)
  - [10.2 知道 golang 的内存逃逸吗？什么情况下会发生内存逃逸？](#102-知道-golang-的内存逃逸吗什么情况下会发生内存逃逸)
  - [10.3 内存逃逸有什么影响？](#103-内存逃逸有什么影响)
  - [10.4 Channel是分配在栈上，还是堆上？](#104-channel是分配在栈上还是堆上)
  - [10.5 Go语言在什么情况下会发生内存泄漏？](#105-go语言在什么情况下会发生内存泄漏)
  - [10.6 Go语言发生了内存泄漏如何定位和优化？](#106-go语言发生了内存泄漏如何定位和优化)
- [11. 垃圾回收面试题](#11-垃圾回收面试题)
  - [11.1 常见的 GC 实现方式有哪些？](#111-常见的-gc-实现方式有哪些)
  - [11.2 Go 语言的 GC 使用的是什么？](#112-go-语言的-gc-使用的是什么)
  - [11.3 三色标记法是什么？](#113-三色标记法是什么)
  - [11.4 Go语言GC的根对象到底是什么？](#114-go语言gc的根对象到底是什么)
  - [11.5 STW 是什么意思？](#115-stw-是什么意思)
  - [11.6 并发标记清除法的难点是什么？](#116-并发标记清除法的难点是什么)
  - [11.7 Go语言是如何解决并发标记清除时，用户程序并发修改对象引用问题的？](#117-go语言是如何解决并发标记清除时用户程序并发修改对象引用问题的)
  - [11.8 什么是写屏障、混合写屏障，如何实现？](#118-什么是写屏障混合写屏障如何实现)
  - [11.9 Go 语言中 GC 的流程是什么？](#119-go-语言中-gc-的流程是什么)
  - [11.10 GC触发的时机有哪些？](#1110-gc触发的时机有哪些)
  - [11.11 GC 关注的指标有哪些？](#1111-gc-关注的指标有哪些)
  - [11.12 有了 GC，为什么还会发生内存泄露？](#1112-有了-gc为什么还会发生内存泄露)
  - [11.13 Go 的 GC 如何调优？](#1113-go-的-gc-如何调优)
  - [11.14 如何观察 Go GC？](#1114-如何观察-go-gc)
- [12. Go代码面试题](#12-go代码面试题)
  - [12.1 开启100个协程，顺序打印1-1000，且保证协程号1的，打印尾数为1的数字](#121-开启100个协程顺序打印1-1000且保证协程号1的打印尾数为1的数字)
  - [12.2 三个goroutinue交替打印abc 10次](#122-三个goroutinue交替打印abc-10次)
  - [12.3 用不超过10个goroutine不重复的打印slice中的100个元素](#123-用不超过10个goroutine不重复的打印slice中的100个元素)
  - [12.4 两个协程交替打印奇偶数](#124-两个协程交替打印奇偶数)
  - [12.5 用单个channel实现0,1的交替打印](#125-用单个channel实现01的交替打印)
  - [12.6 sync.Cond实现多生产者多消费者](#126-synccond实现多生产者多消费者)
  - [12.7 使用go实现1000个并发控制并设置执行超时时间1秒](#127-使用go实现1000个并发控制并设置执行超时时间1秒)
  - [12.8 使用两个Goroutine，向标准输出中按顺序按顺序交替打出字母与数字，输出是a1b2c3](#128-使用两个goroutine向标准输出中按顺序按顺序交替打出字母与数字输出是a1b2c3)
  - [12.9 编写一个程序限制10个goroutine执行，每执行完一个goroutine就放一个新的goroutine进来](#129-编写一个程序限制10个goroutine执行每执行完一个goroutine就放一个新的goroutine进来)
